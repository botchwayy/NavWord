...
const EndsWith = () => {
    ...
    const [endsWith, setendsWith] = useState("er")

    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await axios.get(`https://api.datamuse.com/words?sp=*${endsWith}&max=${max}`);
    ...
    <View style={styles.container}>
        ...
        <View style={styles.rowContainer}>
            <View style={styles.filterContainer}>
                <Text style={styles.label}>Ends With</Text>
                <TextInput style={styles.input} placeholder='er' value={endsWith} onChangeText={text => setendsWith(text)}/>
            </View>
            <View style={styles.filterContainer}>
                <Text style={styles.label}>Max Words</Text>
                <TextInput style={styles.input} placeholder='20000000000000000000000000' value={max} onChangeText={text => setMax(text)}/>
            </View>
        </View>
